...
// Helper functions to get MetaMask extension URLs
...
function getMetaMaskImportUrl() {
  if (!metamaskExtensionId) throw new Error('MetaMask extension not loaded. Run wallet-setup first with --wallet flag.');
  return `chrome-extension://${metamaskExtensionId}/home.html#onboarding/import-with-recovery-phrase`;
}
...
async function startBrowser(options) {
  ...
  if (useWalletMode) {
    // Wallet mode: Use Playwright's Chromium with MetaMask extension
    const metamaskPath = path.join(EXTENSIONS_DIR, 'metamask');

    if (!fs.existsSync(path.join(metamaskPath, 'manifest.json'))) {
      console.log(JSON.stringify({
        success: false,
        error: 'MetaMask extension not found. Run wallet-setup first.',
    ...
    console.log(JSON.stringify({ status: 'info', message: 'Starting Chromium with MetaMask extension...' }));
    ...
    try {
      ...
      console.log(JSON.stringify({ status: 'info', message: 'MetaMask extension loaded successfully', extensionId: metamaskExtensionId }));
    } catch (e) {
      ...
      try {
        ...
        if (!metamaskExtensionId) {
          console.log(JSON.stringify({ status: 'error', message: 'Could not detect MetaMask extension ID' }));
        }
      } catch (fallbackError) {
        console.log(JSON.stringify({ status: 'error', message: 'Failed to detect extension ID', error: fallbackError.message }));
      }
    }
