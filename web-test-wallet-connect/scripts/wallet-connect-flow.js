 ...
 *
 * Usage:
 *   WALLET_PRIVATE_KEY="0x..." node wallet-connect-flow.js <dapp-url> [options]
 *
 * Options:
...
function runCommand(command, args = [], options = {}) {
  ...
  const fullArgs = [TEST_HELPER, command, ...args, headedFlag, '--wallet', '--keep-open'];
...
async function navigateToDApp() {
  ...
  const result = runCommand('wallet-navigate', [CONFIG.dappUrl], { timeout: 60000 });
...
function detectWalletConnectionNeeded() {
  ...
  const needsConnection = needsConnectionPatterns.some(p => p.test(pageContent));
...
function verifyWalletConnection() {
  ...
  if (hasSuccessIndicator && !hasFailureIndicator) {
    console.log('[VERIFY] Wallet connection VERIFIED - success indicators found');
  ...
  if (!hasFailureIndicator && !hasSuccessIndicator) {
    ...
    console.log('[VERIFY] Connection status ambiguous, checking button state...');

    // Try to find connect wallet button
    const listResult = runCommand('list-elements', []);
    const elements = listResult.elements || [];

    const connectButton = elements.find(el =>
      /connect.*wallet/i.test(el.text) ||
      /sign\s*in/i.test(el.text)
    );
  ...
  console.log('[VERIFY] Wallet connection FAILED - still showing connection UI');
...
async function connectWalletWithRetry() {
  ...
  while (attempt < CONFIG.maxRetries) {
    ...
    try {
      ...
      console.log('2. In Rainbow/Privy modal, click "Installed" or wallet icon');
      ...
      console.log('Screenshot saved to: ./test-output/screenshots/connection-check.jpg\n');
...
async function main() {
  ...
  if (!CONFIG.dappUrl) {
    ...
    console.error('Usage: WALLET_PRIVATE_KEY="0x..." node wallet-connect-flow.js <dapp-url>');
    process.exit(2);
  }
